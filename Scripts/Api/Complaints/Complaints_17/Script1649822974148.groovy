...
maps = [('complaintIdsClassify') : complaintIdsClassify1]

ResponseObject response1 = WS.sendRequestAndVerify(findTestObject('Object Repository/Api/Complaints/Search complaint by id', maps), FailureHandling.STOP_ON_FAILURE)
println response1.responseText
WS.verifyMatch(response1.statusCode.toString(), statusCode, false, FailureHandling.STOP_ON_FAILURE)
WS.verifyMatch(WS.getElementPropertyValue(response1, 'data.id').toString(), complaintIdsClassify1, false, FailureHandling.CONTINUE_ON_FAILURE)
