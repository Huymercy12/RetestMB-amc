...
Map map = ['name' : name, 'otherOpinion' : otherOpinion,'id' : id,'label' : label, 'name_q' : name_q, 'content' : content , 'id_q' : id_q, 'label_q' : label_q,'type' : type,'rangeAnswer' : rangeAnswer]

String rs = Statemnet.getTotalRecorded('EVO_SURVEY.SURVEY_TEMPLATE')

int totalRecord1 = Integer.parseInt(rs)

ResponseObject response = WS.sendRequestAndVerify(findTestObject('Object Repository/Api/Survey/Create Survey Template', 
