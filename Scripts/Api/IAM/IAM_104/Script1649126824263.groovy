...
while (rs.next()) {
    ...
    ResponseObject response = WS.sendRequestAndVerify(findTestObject('Object Repository/Api/IAM/Active User', maps), FailureHandling.STOP_ON_FAILURE)
...
while (rs2.next()) {
    WS.verifyMatch(rs2.getString('STATUS'), 'ACTIVE', false)
}
