...
Map map = DataExcelUtils.getDataByIndex(1, 'Data Files/Api/IAM/Create Organization Contact')
WS.callTestCase(findTestCase('Test Cases/Api/IAM/IAM_176'), map)

String organizationId = VariableCollections.map.get('id')
...
Map maps = [('organizationId') : organizationId, ('dayOfBirth') : dayOfBirthRandom, ('gender') : gender, ('fullName') : fullNameRandom
    , ('email') : emailRandom, ('phoneNumber') : phoneNumberRandom, ('contactsId') : GlobalVariable.ID_NEW_ORGANIZATION_CONTACT]

ResponseObject response = WS.sendRequestAndVerify(findTestObject('Object Repository/Api/IAM/Update Organization Contact', maps), FailureHandling.STOP_ON_FAILURE)
...
ResultSet rs = Statemnet.getNewOrganizationContact(GlobalVariable.ACCOUNT_ADMIN.get('username'))
...
while (rs.next()) {
    ...
    GlobalVariable.ID_NEW_ORGANIZATION_CONTACT = rs.getString('ID')
}
