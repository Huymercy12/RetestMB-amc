...
for (WebElement row : rows) {
	...
	actions.moveToElement(listButton[1]).perform()
	if (AbstractWeb.getText('Object Repository/Web/DashboardPage/tooltipContent') == 'Xóa') {
		actions.click().perform()
		//Verify popup delete floor
		AbstractWeb.verifyText('Object Repository/Web/Buildings/titlePopup', titleDeleteFloor)
		AbstractWeb.verifyText('Object Repository/Web/Buildings/contentPopup', contentDeleteFloor)
		...
		AbstractWeb.verifyMessage('Object Repository/Web/message', msgDeleteSuccess)
	}
	
	ResultSet rs2 = Statemnet.getFloor(buildingId)
	while (rs2.next()) {
		WebUI.verifyMatch('1', rs2.getString('DELETED'), false, FailureHandling.STOP_ON_FAILURE)
	}
	ConnectionDatabase.closeDatabaseConnection()
}